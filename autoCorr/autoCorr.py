...
# here we set the format that we want to have for the recorded audio file that we save
# i.e. stereo or mono what sampling rate to use and how many seconds of recording we want
...
# here i read the audio file
audioFile = read("file.wav")
# saves the raw data from the audio file to later display
audioInputData = audioFile[1]
# converts the audio file into an array
audioFileArray = np.array(audioFile[1])
#audioFileArray = np.hstack(audioFile[1])
print(audioFileArray)

# load the audio file again using libroasa library this is to be able to use it for auto correlation
...
# 1) plot is the audio file with the banpass filter applied 2) is the audio file displayed
# 3) is the result from the autocorrelation 4) is the raw data from the file displayed
